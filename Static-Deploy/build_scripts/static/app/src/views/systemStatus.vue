...
function transformApiData(apiData) {
	return {
		CPU: apiData.cpu || {},
		Memory: {
			VirtualMemory: apiData.memory.virtual_memory || {},
			SwapMemory: apiData.memory.swap_memory || {}
		},
		Disks: apiData.disk
			? Object.keys(apiData.disk).map(mountPoint => ({
				mountPoint,
				...apiData.disk[mountPoint]
			}))
			: [],
		NetworkInterfaces: apiData.network
			? Object.fromEntries(
				Object.entries(apiData.network).map(([key, stats]) => [
					key,
					{
						bytes_sent: stats.byte_sent,
						bytes_recv: stats.byte_recv
					}
				])
			)
			: {},
		Processes: apiData.process || {}
	};
}
...
const getData = () => {
	fetchGet("/api/systemStatus", {}, (res) => {
		...
		historicalVirtualMemoryUsage.value.push(transformedData.Memory.VirtualMemory.percent)
		historicalSwapMemoryUsage.value.push(
			transformedData.Memory.SwapMemory ? transformedData.Memory.SwapMemory.percent : 0
		)

		historicalNetworkData.value.bytes_recv.push(
			Object.values(transformedData.NetworkInterfaces)
				.map(x => x.bytes_recv)
