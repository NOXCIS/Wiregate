        ...
        try:
            ...
            if self.Name in stats.keys():
                ...
                if self.Name in stats.keys():
                    ...
                    # Calculate MB/s
                    net_in = round((recv2 - recv1) / 1024 / 1024, 3)  # Convert to MB/s
                    net_out = round((sent2 - sent1) / 1024 / 1024, 3)  # Convert to MB/s
                    
                    print(f"[DEBUG] Interface {self.Name} traffic: in={net_in}MB/s, out={net_out}MB/s")  # Debug log
                    ...
                    print(f"[DEBUG] Interface {self.Name} not found in second measurement")
                    return {"sent": 0, "recv": 0}
            else:
                print(f"[DEBUG] Interface {self.Name} not found in first measurement")
                return {"sent": 0, "recv": 0}
        except Exception as e:
