	...
	<div class="container-fluid d-flex align-items-center justify-content-center min-vh-100 position-relative">
		...
		<div class="text-center error-content">
			...
			<div class="error-message mb-4">
				<h2 class="h3 mb-3">429 Too Many Requests</h2>
				<p class="mb-0">You've exceeded the rate limit. Please slow down.</p>
			</div>
			<div class="error-actions">
				...
				<button 
					@click="retryAfterDelay" 
					class="btn btn-info btn-lg"
					type="button"
					:disabled="isRetrying"
				>
					<i class="bi bi-arrow-clockwise me-2" :class="{ 'spinning': isRetrying }"></i>
					...
					<div class="col-md-8">
						<div class="card border-0 shadow-sm">
							<div class="card-body">
								...
								<p class="card-text">You've made too many requests in a short period. Please wait before trying again.</p>
								<ul class="list-unstyled">
									<li><i class="bi bi-check-circle text-success me-2"></i>Wait a few moments before retrying</li>
									<li><i class="bi bi-check-circle text-success me-2"></i>Reduce the frequency of your requests</li>
									<li><i class="bi bi-check-circle text-success me-2"></i>Contact support if this persists</li>
								</ul>
								<div v-if="retryAfter" class="mt-3">
									<div class="alert alert-info">
										<i class="bi bi-clock me-2"></i>
										You can try again in <strong>{{ retryAfter }} seconds</strong>
									</div>
								</div>
							</div>
						</div>
...
export default {
	name: 'TooManyRequests',
		...
		// Check for retry_after time in URL parameters
		const getRetryAfterTime = () => {
			const urlParams = new URLSearchParams(window.location.search)
			const retryAfterParam = urlParams.get('retry_after')
			
			if (retryAfterParam) {
				retryAfter.value = parseInt(retryAfterParam)
			} else {
				// Default fallback
				retryAfter.value = 30
			}
	...
	methods: {
		initMatrixRain() {
			...
			const getRandomText = () => {
				// Easter egg words with low probability - updated for 429 theme
				const easterEggs = ['slow down', 'rate limit', 'too fast', 'throttled', '429', 'wait', 'patience', 'calm down', 'breathe'];
